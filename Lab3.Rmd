...
6. Making graphical predictions of the results of logistic regression models for our uni-variate covariates. 

### Preliminaries: getting started, loading packages, setting working directory
Today we will use the following packages `packages <- c("ggplot2","lattice", "tidyverse", "effects")` - note I suppress output in the next set of ipak functions. 
...
packages <- c("ggplot2","lattice", "tidyverse", "effects", "here")
...
Lets rescale to be the odds in a 100 meter change of elevation. 
```{r}
...
Now lets go back to our elevation object and save predictions from the Elevation model in the wolf_all dataframe. We can use this to plot predicted values for most situations as well. 
```{r}
## back to elevation
elev <- glm(used ~ Elevation2, family=binomial(logit), data=wolf_all)
summary(elev)
